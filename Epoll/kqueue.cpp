/*************************************************************************
	...
	> Created Time: 一 11/ 4 09:20:08 2019
 ************************************************************************/
...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        if (nev < 0) diep("kevent()");
        else if (nev > 0) {
            if (evlist[0].flags & EV_EOF) /* 读取socket关闭指示 */
                exit(EXIT_FAILURE);
            
            for (i = 0; i < nev; i++) {
                if (evlist[i].flags & EV_ERROR) { /* 报告错误 */
                    fprintf(stderr, "EV_ERROR: %s\n", strerror(evlist[i].data));
                    exit(EXIT_FAILURE);
                }                         
                if (evlist[i].ident == sckfd) { /* 我们从host接收到数据 */
                    memset(buf, 0, BUFSIZE);
                    if (read(sckfd, buf, BUFSIZE) < 0) diep("read()");
                    fputs(buf, stdout);
                } else if (evlist[i].ident == fileno(stdin)) { /* stdin中有数据输入 */
                    memset(buf, 0, BUFSIZE);
